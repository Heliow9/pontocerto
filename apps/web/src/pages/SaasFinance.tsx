...
export function SaasFinance({view}:{view:View}){
  ...
  return <>
    <div className="commercial-heading"><div><span className="commercial-kicker">Financeiro</span><h1>{titles[view][0]}</h1><p>{titles[view][1]}</p></div><div className="commercial-heading-actions">
      ...
      {view!=="logs"&&<button className="primary" onClick={()=>setCreateOpen(true)}>+ Nova cobrança</button>}
    </div></div>
    ...
    {loading&&!dashboard&&rows.length===0?<div className="panel">Carregando financeiro…</div>:
      ...
      <Charges rows={rows} tenants={tenants} filters={filters} setFilters={setFilters} onAction={chargeAction} onProfile={setProfileTenant} onEmail={setEmailCharge} onHistory={setHistoryCharge} busy={busy}/>
    }
...
function CreateCharge({tenants,onClose,onDone,onEditProfile}:{tenants:any[];onClose:()=>void;onDone:()=>Promise<void>;onEditProfile:(tenantId:number)=>void}){
  ...
  const [form,setForm]=useState<any>({
    tenantId:tenants[0]?.id||"",type:"MONTHLY",competence:competence(),amount:null as number|null,dueDate:today(),description:"",issue:true,provider:"",paymentMethod:"",payerSource:"TENANT",sendEmailAfterIssue:true,
    payer:{personType:"PJ",name:"",document:"",email:"",phone:"",zipCode:"",street:"",number:"",complement:"",district:"",city:"",state:""}
  });
  const [busy,setBusy]=useState(false),[error,setError]=useState("");

  useEffect(()=>{void api.get("/saas/finance/providers").then(r=>{setSettings(r.data);setForm((f:any)=>({...f,provider:r.data.defaultProvider||"",paymentMethod:r.data.defaultMethod||""}));}).catch(e=>setError(apiMessage(e)));},[]);
  ...
  const tenantMode=form.type!=="AD_HOC"||form.payerSource==="TENANT";
  useEffect(()=>{
  ...
  const effectiveProvider=form.type==="AD_HOC"?form.provider:settings?.defaultProvider;
  const effectiveMethod=form.type==="AD_HOC"?form.paymentMethod:settings?.defaultMethod;
  ...
  if(tenantMode){
    ...
    if(profile){
      ...
      const doc=digits(profile.billingDocument);if(![11,14].includes(doc.length))validationErrors.push("CPF/CNPJ");
    ...
    const doc=digits(p.document);if((p.personType==="PF"&&doc.length!==11)||(p.personType==="PJ"&&doc.length!==14))validationErrors.push(p.personType==="PF"?"CPF":"CNPJ");
  ...
  async function submit(e:React.FormEvent){
    ...
    try{
      ...
      if(form.type==="MONTHLY")await api.post("/saas/finance/charges/monthly",{tenantId:Number(form.tenantId),competence:form.competence,issue:form.issue,sendEmailAfterIssue:form.sendEmailAfterIssue});
  ...
  const defaultText=settings?.defaultProvider&&settings?.defaultMethod?`${providerLabels[settings.defaultProvider]} · ${methodLabel[settings.defaultMethod]}`:"Padrão ainda não configurado";
  ...
  return <Modal title="Nova cobrança" onClose={onClose} wide><form className="commercial-form" onSubmit={submit}><div className="commercial-form-grid">
    <label>Tipo<select value={form.type} onChange={e=>setForm({...form,type:e.target.value,payerSource:e.target.value==="AD_HOC"?form.payerSource:"TENANT"})}><option value="MONTHLY">Mensalidade</option><option value="IMPLEMENTATION">Implantação</option><option value="AD_HOC">Cobrança avulsa</option></select></label>
    ...
    {form.type==="AD_HOC"&&form.payerSource==="EXTERNAL"&&<>
      ...
      <label>CEP<MaskedInput mask="cep" value={form.payer.zipCode} onChange={zipCode=>updatePayer({zipCode})}/></label>
  ...
  <div className="form-actions"><button type="button" className="ghost" onClick={onClose}>Cancelar</button><button className="primary" disabled={busy||!settings||uniqueErrors.length>0}>{busy?"Gerando…":"Gerar cobrança"}</button></div></form></Modal>;
}
