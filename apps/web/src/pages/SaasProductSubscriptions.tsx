...
export function SaasProductSubscriptions(){
  const [rows,setRows]=useState<any[]>([]),[products,setProducts]=useState<any[]>([]),[editing,setEditing]=useState<any>(null),[error,setError]=useState(''),[saving,setSaving]=useState(false),[loading,setLoading]=useState(true);
  const load=async()=>{setLoading(true);setError('');try{const [subscriptions,productCatalog]=await Promise.all([api.get('/saas/product-subscriptions'),api.get('/saas/products')]);setRows(subscriptions.data);setProducts(productCatalog.data);}catch(e){setError(apiMessage(e));}finally{setLoading(false);}};
  useEffect(()=>{void load();},[]);
  ...
  async function openEdit(row:any){
    ...
    try{
      ...
      setEditing({
        ...
        billingSource:data.billing_source,status:data.status,legacyMigrated:Boolean(data.legacy_migrated)
      });
  ...
  async function save(){
    ...
    try{
      await api.put(`/saas/product-subscriptions/${editing.id}`,{
        ...
        firstCycleProrataEnabled:editing.legacyMigrated?false:Boolean(editing.firstCycleProrataEnabled),financialContactEmail:String(editing.financialContactEmail||'').trim()||null
      });
  ...
  return <><PageHeader title="Assinaturas por Produto" subtitle="Planos, preços, vencimentos e integração operacional de cada produto contratado."/>
    ...
    {editing&&<Modal title={`Editar assinatura · ${editing.customerName}`} onClose={()=>!saving&&setEditing(null)} wide><div className="commercial-settings-intro">Alterações de plano, valor, desconto, vencimento e e-mail financeiro valem para as próximas cobranças. Uma cobrança já emitida não é modificada automaticamente; use “Ajustar / reemitir” no Financeiro para cancelar e gerar outra.</div><div className="form-grid">
      ...
      <label className="checkbox-row"><input type="checkbox" disabled={editing.legacyMigrated} checked={editing.legacyMigrated?false:editing.firstCycleProrataEnabled} onChange={e=>setEditing({...editing,firstCycleProrataEnabled:e.target.checked})}/> Pró-rata automático na primeira cobrança</label>{editing.legacyMigrated&&<div className="commercial-settings-intro span-2">Assinatura migrada do Movyo legado: o pró-rata retroativo permanece desabilitado. Alterações de plano não transformam esta assinatura em uma nova adesão.</div>}
