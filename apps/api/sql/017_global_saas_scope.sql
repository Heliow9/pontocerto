...
UPDATE commercial_proposals cp JOIN plans p ON p.id=cp.plan_id SET cp.plan_name_snapshot=COALESCE(cp.plan_name_snapshot,p.name),cp.proposal_number=COALESCE(cp.proposal_number,CONCAT('PC-',YEAR(cp.created_at),'-',LPAD(cp.id,5,'0'))),cp.commercial_snapshot_json=COALESCE(cp.commercial_snapshot_json,JSON_OBJECT('planName',p.name,'maxEmployees',cp.max_employees,'priceMonthly',cp.price_monthly,'maxBranches',cp.max_branches,'features',cp.features_json,'implementationDays',cp.implementation_days,'implementationFee',cp.implementation_fee));
