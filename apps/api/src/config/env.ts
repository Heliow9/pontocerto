...
const schema = z.object({
  ...
  MOVYO_BRIDGE_ENABLED: z.enum(["0","1"]).default("0"),
  MOVYO_BRIDGE_BASE_URL: z.preprocess(emptyToUndefined, z.string().url().optional()),
  MOVYO_BRIDGE_CLIENT_ID: z.string().default("ponto-certo"),
  MOVYO_BRIDGE_SECRET: z.preprocess(emptyToUndefined, z.string().min(16).optional()),
  MOVYO_BRIDGE_TIMEOUT_MS: z.coerce.number().int().min(1000).max(60000).default(10000),
  MOVYO_SYNC_WORKER_ENABLED: z.enum(["0","1"]).default("0"),
