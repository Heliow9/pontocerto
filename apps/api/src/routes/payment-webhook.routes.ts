...
paymentWebhookRouter.post("/mercadopago",async(req,res,next)=>{try{const result=await processMercadoPagoWebhook(req.body,req.headers as any,req.query as any);res.status(result.status==="FAILED"?202:200).json({ok:result.status!=="FAILED",result});}catch(e){next(e);}});
