import { checkBranchCapacity } from "../services/entitlements.service.js";
...
companiesRouter.post("/",requireRole("SUPER_ADMIN","TENANT_ADMIN"),async(req,res)=>{
  ...
  const conn=await pool.getConnection();try{await conn.beginTransaction();await conn.query("SELECT id FROM tenants WHERE id=? FOR UPDATE",[req.auth!.tenantId]);if(d.active)await checkBranchCapacity(req.auth!.tenantId,conn);const [result]=await conn.query<any>(`INSERT INTO companies (tenant_id,company_type,legal_name,trade_name,cnpj,active,created_at,updated_at) VALUES (?,'BRANCH',?,?,?,?,${BRASILIA_NOW_SQL},${BRASILIA_NOW_SQL})`,[req.auth!.tenantId,d.legalName,d.tradeName||null,d.cnpj||null,d.active?1:0]);
    await conn.query(`INSERT INTO company_profiles (${profileCols}) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,${BRASILIA_NOW_SQL},${BRASILIA_NOW_SQL})`,[req.auth!.tenantId,result.insertId,d.phone||null,d.email||null,geo.address,d.zipCode||null,d.street||null,d.number||null,d.complement||null,d.district||null,d.city||null,d.state||null,(d.country||"BR").toUpperCase(),geo.latitude,geo.longitude,d.punchRadiusMeters,d.requireLocation?1:0,d.blockOutsideRadius?1:0,d.maxGpsAccuracyMeters,d.requireDeviceBiometric?1:0,d.requireRegisteredDevice?1:0,d.maxRegisteredDevices,d.enforceScheduleWindow?1:0,d.scheduleEarlyMarginMinutes,d.scheduleLateMarginMinutes,d.requireFaceRecognition?1:0,geo.mapboxPlaceId]);
