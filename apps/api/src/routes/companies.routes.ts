...
companiesRouter.post("/",requireRole("SUPER_ADMIN","TENANT_ADMIN"),async(req,res)=>{
  ...
  const conn=await pool.getConnection();try{await conn.beginTransaction();await conn.query("SELECT id FROM tenants WHERE id=? FOR UPDATE",[req.auth!.tenantId]);if(d.active)await checkBranchCapacity(req.auth!.tenantId,conn);const [result]=await conn.query<any>(`INSERT INTO companies (tenant_id,legal_name,trade_name,cnpj,active,created_at,updated_at) VALUES (?,?,?,?,?,${BRASILIA_NOW_SQL},${BRASILIA_NOW_SQL})`,[req.auth!.tenantId,d.legalName,d.tradeName||null,d.cnpj||null,d.active?1:0]);
