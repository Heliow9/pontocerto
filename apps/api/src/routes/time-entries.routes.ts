...
import { evaluateEmployeeGeofence, getCompanySecurityPolicy } from "../services/geofence.service.js";
import { getEmployeeDevicePolicy, validateEmployeeDevice } from "../services/device-biometric.service.js";
import { evaluateEmployeeSchedule } from "../services/schedule-guard.service.js";
import { env } from "../config/env.js";
import { readTimeEntrySelfie, removeTimeEntrySelfie, saveTimeEntrySelfie } from "../services/selfie.service.js";

export const timeEntriesRouter = Router();
timeEntriesRouter.use(authMiddleware);

const entryTypes = ["CLOCK_IN", "BREAK_OUT", "BREAK_IN", "CLOCK_OUT", "OTHER"] as const;
...
async function getEmployeeForClock(req: any, requestedEmployeeId?: number) {
  const employeeId = req.auth.role === "FUNCIONARIO" ? req.auth.employeeId : requestedEmployeeId;
...
async function logAttempt(args: {
  ...
  try {
    await pool.query(
      ...
      [
        args.tenantId,args.companyId,args.employeeId,args.latitude??null,args.longitude??null,args.accuracy??null,
        args.mocked==null?null:args.mocked?1:0,args.geoDecision||null,
        args.distance==null?null:Math.round(args.distance*100)/100,args.biometricDecision||null,
        args.scheduleDecision||null,args.deviceUid||null,args.success?1:0,args.reason||null
      ]
    );
  } catch (error) {
...
timeEntriesRouter.post("/geofence-preview", async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ message: "Coordenadas inválidas." });
...
timeEntriesRouter.post("/secure", selfieUpload.single("selfie"), async (req, res) => {
  if (req.auth!.role !== "FUNCIONARIO" || !req.auth!.employeeId) {
    return res.status(403).json({ message: "Registro seguro disponível para usuário funcionário." });
  }
  const parsed = secureSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: "Dados do registro inválidos.", issues: parsed.error.flatten() });
  if (!req.file) {
    return res.status(400).json({
      message:"A foto do funcionário é obrigatória antes do registro de ponto.",
  ...
  if (schedule.decision === "BLOCKED") {
    await logAttempt({
      tenantId:req.auth!.tenantId,companyId:employee.company_id,employeeId:employee.id,
      latitude:d.latitude,longitude:d.longitude,accuracy:d.accuracy,mocked:d.locationMocked,
      scheduleDecision:schedule.decision,biometricDecision:"NOT_CHECKED",deviceUid:d.deviceUid,
      success:false,reason:schedule.message
    });
    return res.status(403).json({ message:schedule.message, code:"SCHEDULE_BLOCKED", schedule });
  }

  if (schedule.complete) {
    const message = "A jornada de hoje já possui todas as marcações previstas. Solicite um ajuste ao RH se precisar corrigir o ponto.";
    await logAttempt({
      tenantId:req.auth!.tenantId,companyId:employee.company_id,employeeId:employee.id,
      latitude:d.latitude,longitude:d.longitude,accuracy:d.accuracy,mocked:d.locationMocked,
      scheduleDecision:"BLOCKED",biometricDecision:"NOT_CHECKED",deviceUid:d.deviceUid,
      success:false,reason:message
    });
    return res.status(409).json({ message, code:"SCHEDULE_COMPLETE", schedule });
  }
  ...
  if (geo.decision === "BLOCKED") {
    await logAttempt({
      tenantId:req.auth!.tenantId,companyId:employee.company_id,employeeId:employee.id,
      latitude:d.latitude,longitude:d.longitude,accuracy:d.accuracy,mocked:d.locationMocked,
      geoDecision:geo.decision,distance:geo.distanceMeters,scheduleDecision:schedule.decision,
      biometricDecision:"NOT_CHECKED",deviceUid:d.deviceUid,success:false,reason:geo.message
    });
    return res.status(403).json({ message:geo.message, code:"GEOFENCE_BLOCKED", geo, schedule });
  }

  const device = await validateEmployeeDevice({
    tenantId:req.auth!.tenantId,companyId:employee.company_id,employeeId:employee.id,
    deviceUid:d.deviceUid,deviceSecret:d.deviceSecret
  });
  if (!device.verified) {
    await logAttempt({
      tenantId:req.auth!.tenantId,companyId:employee.company_id,employeeId:employee.id,
      latitude:d.latitude,longitude:d.longitude,accuracy:d.accuracy,mocked:d.locationMocked,
      geoDecision:geo.decision,distance:geo.distanceMeters,scheduleDecision:schedule.decision,
      biometricDecision:"REJECTED",deviceUid:d.deviceUid,success:false,reason:device.message
    });
    return res.status(403).json({ message:device.message, code:device.code||"DEVICE_REJECTED", schedule });
  }

  const entryType = (schedule.nextType || d.type || "OTHER") as typeof entryTypes[number];
  const biometricVerified = device.policy.requireDeviceBiometric ? true : null;
  ...
  try {
    ...
    const [result] = await conn.query<any>(
      `INSERT INTO time_entries
       ...
       VALUES (?,?,?, ?,${BRASILIA_NOW_SQL},?,?,?,?, 'MOBILE',0,?,${BRASILIA_NOW_SQL},?,?,?,?,?,?)`,
      [
        req.auth!.tenantId,employee.company_id,employee.id,entryType,d.latitude,d.longitude,d.accuracy??null,
        d.deviceUid,req.auth!.userId,d.locationMocked?1:0,biometricVerified==null?null:1,
        device.policy.requireDeviceBiometric?d.biometricType:null,device.device?.id||null,
        schedule.decision,schedule.workDate||null
      ]
    );
    ...
    await conn.query(
      ...
      [req.auth!.tenantId,timeEntryId,geo.location?.id||null,geo.distanceMeters==null?null:Math.round(geo.distanceMeters*100)/100,geo.withinRadius==null?null:geo.withinRadius?1:0,geo.decision]
    );
    ...
    await conn.query(
      `INSERT INTO time_entry_device_checks
       ...
       VALUES (?,?,?,?,?,?,?,?,${BRASILIA_NOW_SQL})`,
      [
        req.auth!.tenantId,timeEntryId,employee.id,device.device?.id||null,d.deviceUid||null,
    ...
    await conn.query(
      `INSERT INTO time_entry_schedule_checks
       ...
       VALUES (?,?,?,?,?,?,?,?,?,${BRASILIA_NOW_SQL})`,
      [req.auth!.tenantId,timeEntryId,employee.id,schedule.scheduleId||null,schedule.workDate||null,schedule.weekday??null,schedule.scheduleText||null,entryType,schedule.decision]
    );
    ...
    await conn.query(
      `INSERT INTO time_entry_selfies
       ...
       VALUES (?,?,?,?,?,?,?,?,${BRASILIA_NOW_SQL},${BRASILIA_NOW_SQL})`,
      [
        req.auth!.tenantId,employee.company_id,employee.id,timeEntryId,selfie.relativePath,
        req.file!.mimetype,selfie.fileSize,selfie.sha256
      ]
    );
    ...
    await logAttempt({
      ...
      latitude:d.latitude,longitude:d.longitude,accuracy:d.accuracy,mocked:d.locationMocked,
      geoDecision:geo.decision,distance:geo.distanceMeters,scheduleDecision:schedule.decision,
      biometricDecision:device.policy.requireDeviceBiometric?"VERIFIED":device.policy.employeeBiometricExempt?"EXEMPT":"NOT_REQUIRED",
      deviceUid:d.deviceUid,success:true
    });
...
timeEntriesRouter.post("/", async (req, res) => {
  ...
  if(req.auth!.role==="FUNCIONARIO"&&(policy.requireDeviceBiometric||policy.requireRegisteredDevice)) {
    return res.status(403).json({message:"Esta empresa exige biometria e aparelho vinculado. Use o registro seguro do aplicativo.",code:"SECURE_PUNCH_REQUIRED"});
  }

  const geo=await evaluateEmployeeGeofence({tenantId:req.auth!.tenantId,companyId:employee.company_id,employeeId:employee.id,latitude:d.latitude,longitude:d.longitude,accuracy:d.accuracy,mocked:d.locationMocked});
  if(geo.decision==="BLOCKED") return res.status(403).json({message:geo.message,geo});
  ...
  const [result]=await pool.query<any>(
    ...
    [req.auth!.tenantId,employee.company_id,employee.id,d.type,d.latitude||null,d.longitude||null,d.accuracy||null,d.deviceId||null,d.source,req.auth!.userId,d.locationMocked==null?null:d.locationMocked?1:0]
  );
  await pool.query(
    `INSERT INTO time_entry_geo_checks (tenant_id,time_entry_id,work_location_id,distance_meters,within_radius,decision,created_at)
     VALUES (?,?,?,?,?,?,${BRASILIA_NOW_SQL})`,
    [req.auth!.tenantId,result.insertId,geo.location?.id||null,geo.distanceMeters==null?null:Math.round(geo.distanceMeters*100)/100,geo.withinRadius==null?null:geo.withinRadius?1:0,geo.decision]
  );
  const [rows]=await pool.query<any[]>("SELECT id,registered_at,entry_type,latitude,longitude FROM time_entries WHERE id=? AND tenant_id=?",[result.insertId,req.auth!.tenantId]);
  res.status(201).json({...rows[0],geo});
});

timeEntriesRouter.get("/my/today", async (req,res) => {
  if(!req.auth!.employeeId) return res.status(400).json({message:"Usuário não vinculado a funcionário."});
  const [rows]=await pool.query<any[]>(
...
timeEntriesRouter.get("/", requireRole("SUPER_ADMIN","TENANT_ADMIN","RH","GESTOR","SUPERVISOR"), async (req,res) => {
  const start=String(req.query.start||""),end=String(req.query.end||start),employeeId=req.query.employeeId?Number(req.query.employeeId):null;
  const params:any[]=[req.auth!.tenantId],clauses:string[]=[];
  if(start){clauses.push("DATE(te.registered_at)>=?");params.push(start)}
  if(end){clauses.push("DATE(te.registered_at)<=?");params.push(end)}
  if(employeeId){clauses.push("te.employee_id=?");params.push(employeeId)}
  const [rows]=await pool.query<any[]>(
...
timeEntriesRouter.get("/:id/selfie", async (req,res)=>{
  ...
  const selfie=rows[0];
  if(!selfie) return res.status(404).json({message:"Foto da marcação não encontrada."});

  if(req.auth!.role==="FUNCIONARIO" && Number(req.auth!.employeeId)!==Number(selfie.employee_id)){
    return res.status(403).json({message:"Sem permissão para visualizar esta foto."});
  }
...
const manualSchema=z.object({employeeId:z.number().int().positive(),registeredAt:z.string().min(16),type:z.enum(entryTypes).default("OTHER"),reason:z.string().min(3).max(500)});
timeEntriesRouter.post("/manual",requireRole("SUPER_ADMIN","TENANT_ADMIN","RH","GESTOR"),async(req,res)=>{
  const parsed=manualSchema.safeParse(req.body);if(!parsed.success)return res.status(400).json({message:"Dados inválidos."});
  const d=parsed.data;
  const [employees]=await pool.query<any[]>("SELECT id,company_id FROM employees WHERE id=? AND tenant_id=? AND active=1 LIMIT 1",[d.employeeId,req.auth!.tenantId]);
  if(!employees[0])return res.status(404).json({message:"Funcionário não encontrado."});
  ...
  const [result]=await pool.query<any>(
    ...
    [req.auth!.tenantId,employees[0].company_id,d.employeeId,d.type,normalized,d.reason,req.auth!.userId]
  );
  await writeAudit(req,"MANUAL_CREATE","time_entry",Number(result.insertId),undefined,d);
  res.status(201).json({id:Number(result.insertId)});
});

timeEntriesRouter.delete("/:id",requireRole("SUPER_ADMIN","TENANT_ADMIN","RH"),async(req,res)=>{
  const id=Number(req.params.id);
  const [rows]=await pool.query<any[]>("SELECT * FROM time_entries WHERE id=? AND tenant_id=? LIMIT 1",[id,req.auth!.tenantId]);
  if(!rows[0])return res.status(404).json({message:"Registro não encontrado."});
  await pool.query("DELETE FROM time_entries WHERE id=? AND tenant_id=?",[id,req.auth!.tenantId]);
