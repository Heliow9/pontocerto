...
assert.equal(exists('/api/sql/015_system_logs.sql'), true, 'migration 015 missing');
assert.equal(exists('/api/src/services/system-log.service.ts'), true, 'system log service missing');
assert.equal(exists('/api/src/routes/logs.routes.ts'), true, 'logs route missing');
assert.equal(exists('/web/src/components/CompanyLogs.tsx'), true, 'CompanyLogs UI missing');

const migration = read('/api/sql/015_system_logs.sql');
...
const service = read('/api/src/services/system-log.service.ts');
...
const routes = read('/api/src/routes/logs.routes.ts');
...
const whatsapp = read('/api/src/services/whatsapp.service.ts');
...
const companyPage = read('/web/src/pages/CompaniesPage.tsx');
assert.ok(companyPage.includes('CompanyLogs'), 'CompanyLogs not integrated');

const ui = read('/web/src/components/CompanyLogs.tsx');
