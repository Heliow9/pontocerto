...
test('025 allows product monthly billing without fake tenant',()=>{assert.match(sql,/product_subscription_id/);assert.match(sql,/payer_source ENUM\(''TENANT'',''COMMERCIAL'',''EXTERNAL''\)/);assert.doesNotMatch(sql,/CHECK\s*\(/i);assert.match(sql,/tenant_id BIGINT UNSIGNED NULL/);assert.match(sql,/commercial_customer_id BIGINT UNSIGNED NULL/);});
