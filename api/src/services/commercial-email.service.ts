...
export async function sendSmtpMessage(input:{to:string;cc?:string|null;subject:string;text:string;html:string;attachments?:Array<{filename:string;content:Buffer|string;contentType?:string}>;headers?:Record<string,string>}){const smtp=configuredSmtp(await loadEmailSettings());if(smtpConnectionView(smtp).status!=="SUCCESS")throw Object.assign(new Error("Teste e valide a conexão SMTP antes de enviar documentos."),{status:409});const mailer=await nodemailer(),transport=mailer.createTransport(smtpTransportOptions(credentials(smtp)));try{const result=await transport.sendMail({from:{name:smtp.senderName||"PONTO CERTO",address:smtp.senderEmail},to:input.to,cc:input.cc||undefined,subject:input.subject,text:input.text,html:input.html,attachments:input.attachments,headers:input.headers});return {messageId:String(result?.messageId||""),accepted:Array.isArray(result?.accepted)?result.accepted.map(String):[]};}catch(error){throw Object.assign(new Error(sanitizeSmtpError(error)),{status:502});}finally{transport.close?.();}}

export async function sendCommercialEmail(input:{to:string;subject:string;message:string;filename:string;pdf:Buffer;trackingUrl:string;recipientName?:string|null;documentLabel?:string}){const settings=await loadEmailSettings(),smtp=configuredSmtp(settings);return sendSmtpMessage({to:input.to,subject:input.subject,text:input.message,html:buildCommercialEmailHtml({recipientName:input.recipientName,message:input.message,trackingUrl:input.trackingUrl,documentLabel:input.documentLabel||input.filename}),attachments:[{filename:input.filename,content:input.pdf,contentType:"application/pdf"}],headers:buildReadReceiptHeaders(smtp.senderEmail!)});}
